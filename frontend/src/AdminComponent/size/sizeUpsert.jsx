...
const SizeUpsert = (Props) => {
    ...
    async function upsertSize() {
        ...
        const response = await fetch("http://localhost:8800/route/api/addSize", {
            method: "POST",
            headers: {
        ...
        <>
            <div className="flex flex-col justify-center items-center bg-gray-200 ">
                <div className="w-1/2  border-1 border-gray-400 p-4 my-4 rounded-xl bg-white"  >
                    ...
                    <div className="mt-2">
                        <label to="category">Size Name</label>
                        <input  htmlFor="categroy" onChange={(e) => setName(e.target.value)} type="text" name="categoryName" />
                    </div>

                    <div className="text-center">
                        <button onClick={upsertSize} className="py-2 px-4 font-medium border-2 rounded-xl mt-2 text-white center bg-secondary">Add Size</button>
                    </div>
                </div>
            </div>
        </>
